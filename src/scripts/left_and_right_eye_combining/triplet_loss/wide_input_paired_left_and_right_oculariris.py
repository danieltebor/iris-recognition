# -*- coding: utf-8 -*-
"""
Module: resnet50_wide_input_paired_left_and_right_oculariris.py
Author: Daniel Tebor
"""
...
from iris_dataset_processing.triplet_paired_iris_dataset import TripletPairedIrisDataset
from iris_dataset_processing.config.triplet_loss.anchor_left_and_right_oc_oculariris_dataset import *
from iris_dataset_processing.config.training_left_and_right_oc_oculariris_dataset import *
from iris_dataset_processing.config.testing_left_and_right_oc_oculariris_dataset import *
from iris_dataset_processing.config.validation_n50_thru_p50_left_and_right_oc_oculariris_datasets import *
...
INPUT_SHAPE = (3, 448, 224)
MODELNAME = 'resnet50'
MODEL_FILENAME = f'{MODELNAME}_siamese_wide_input_paired_left_and_right_oculariris.pt'
...
def train():
    ...
    left_eye_anchor_dataset = IrisDataset(data_path=ANCHOR_DATASET_PATH,
                                          cohorts=LEFT_EYE_ANCHOR_COHORTS,
                                          img_exclusion_flags=ANCHOR_IMG_EXCLUSION_FLAGS,
                                          img_input_shape=INPUT_SHAPE,
                                          label_encoder=label_encoder,
                                          should_use_augmentation=False)
    right_eye_anchor_dataset = IrisDataset(data_path=ANCHOR_DATASET_PATH,
                                           cohorts=RIGHT_EYE_ANCHOR_COHORTS,
                                           img_exclusion_flags=ANCHOR_IMG_EXCLUSION_FLAGS,
                                           img_input_shape=INPUT_SHAPE,
                                           label_encoder=label_encoder,
                                           should_use_augmentation=False)
    paired_anchor_dataset = DeterministicPairedIrisDataset(left_iris_dataset=left_eye_anchor_dataset, right_iris_dataset=right_eye_anchor_dataset)
    ...
    triplet_training_dataset = TripletPairedIrisDataset(anchor_iris_dataset=paired_anchor_dataset, random_sample_iris_dataset=paired_training_dataset)
    ...
    right_eye_testing_dataset = IrisDataset(data_path=TESTING_DATASET_PATH,
                                            ...
                                            label_encoder=label_encoder,
                                            should_use_augmentation=False)
    paired_testing_dataset = RandomSamplePairedIrisDataset(left_iris_dataset=left_eye_testing_dataset, right_iris_dataset=right_eye_testing_dataset)

    triplet_testing_dataset = TripletPairedIrisDataset(anchor_iris_dataset=paired_anchor_dataset, random_sample_iris_dataset=paired_testing_dataset)
    ...
    model_operator = model_operator_factory.create_model_operator(ModelOperatorName.SIAMESE_WIDE_INPUT)
    ...
    anchor_img_paths = list(paired_anchor_dataset.left_eye_img_paths) + list(paired_anchor_dataset.right_eye_img_paths)
    anchor_imgs = [os.path.join(os.path.basename(os.path.dirname(path)) + '/' + os.path.basename(path)) for path in anchor_img_paths]
    training_img_paths = list(paired_training_dataset.left_iris_dataset.img_paths) + list(paired_training_dataset.right_iris_dataset.img_paths)
...
def _evaluate_accuracy(model_operator: CNNModelOperator,
    ...
    for label, config in VALIDATION_CONFIGS.items():
        ...
        anchor_dataloader = DataLoader(dataset=paired_anchor_dataset,
                                       batch_size=config['batch_size'],
                                       shuffle=False,
                                       num_workers=os.cpu_count())

        left_eye_validation_dataset = IrisDataset(data_path=VALIDATION_DATASET_PATH,
                                                  cohorts=config['left_eye_cohorts'],
                                                  img_exclusion_flags=config['img_exclusion_flags'],
                                                  img_input_shape=INPUT_SHAPE,
                                                  label_encoder=label_encoder)
        right_eye_validation_dataset = IrisDataset(data_path=VALIDATION_DATASET_PATH,
                                                   cohorts=config['right_eye_cohorts'],
                                                   img_exclusion_flags=config['img_exclusion_flags'],
                                                   img_input_shape=INPUT_SHAPE,
                                                   label_encoder=label_encoder)
        paired_validation_dataset = DeterministicPairedIrisDataset(left_iris_dataset=left_eye_validation_dataset, right_iris_dataset=right_eye_validation_dataset)
        
        validation_dataloader = DataLoader(dataset=paired_validation_dataset,
                                           batch_size=config['batch_size'],
                                           shuffle=False,
                                           num_workers=os.cpu_count())
        ...
        anchor_img_paths = list(paired_anchor_dataset.left_eye_img_paths) + list(paired_anchor_dataset.right_eye_img_paths)
        anchor_imgs = [os.path.join(os.path.basename(os.path.dirname(path)) + '/' + os.path.basename(path)) for path in anchor_img_paths]
        validation_img_paths = list(paired_validation_dataset.left_eye_img_paths) + list(paired_validation_dataset.right_eye_img_paths)
...
def _evaluate_accuracy_roc(model_operator: CNNModelOperator,
    ...
    for label, config in VALIDATION_CONFIGS.items():
        ...
        fpr, tpr = model_operator.evaluate_model_roc(anchor_dataloader=anchor_dataloader, validation_dataloader=validation_dataloader)
