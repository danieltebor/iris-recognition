...
Author: Daniel Tebor
Description: This module contains a class for training and evaluating the ResNet50 model with multi-image input acheived by creating a siamese model.
"""
...
class CNNTripletLossWideInputModelOperator(CNNModelOperator):
    def __init__(self,
                 model: nn.Module,
                 modelname: str,
    ...
    def train(self, training_dataloader: DataLoader, testing_dataloader: DataLoader) -> Tuple[int, int]:
        ...
        for epoch in range(1, MAX_EPOCHS):
            ...
            for anchor_left_inputs, anchor_right_inputs, \
                positive_left_inputs, positive_right_inputs, \
                negative_left_inputs, negative_right_inputs, \
                _ in training_dataloader:

                anchor_inputs = []
                positive_inputs = []
                negative_inputs = []

                for anchor_left_input, anchor_right_input, \
                    positive_left_input, positive_right_input, \
                    negative_left_input, negative_right_input \
                    in zip(anchor_left_inputs, anchor_right_inputs, positive_left_inputs, positive_right_inputs, negative_left_inputs, negative_right_inputs):
                    anchor_concatted_input = torch.cat((anchor_left_input, anchor_right_input), dim=1)
                    positive_concatted_input = torch.cat((positive_left_input, positive_right_input), dim=1)
                    negative_concatted_input = torch.cat((negative_left_input, negative_right_input), dim=1)
                    anchor_inputs.append(anchor_concatted_input)
                    positive_inputs.append(positive_concatted_input)
                    negative_inputs.append(negative_concatted_input)

                anchor_inputs = torch.stack(anchor_inputs)
                positive_inputs = torch.stack(positive_inputs)
                negative_inputs = torch.stack(negative_inputs)
    ...
    def evaluate_model_accuracy(self, anchor_dataloader: DataLoader, validation_dataloader: DataLoader) -> float:
        ...
        with torch.no_grad():
            ...
            for left_anchor_inputs, right_anchor_inputs, labels, _, _ in anchor_dataloader:
                anchor_inputs = []

                for left_anchor_input, right_anchor_input in zip(left_anchor_inputs, right_anchor_inputs):
                    concatted_input = torch.cat((left_anchor_input, right_anchor_input), dim=1)
                    anchor_inputs.append(concatted_input)
            ...
            for left_val_inputs, right_val_inputs, labels, _, _ in validation_dataloader:
                validation_inputs = []

                for left_val_input, right_val_input in zip(left_val_inputs, right_val_inputs):
                    concatted_input = torch.cat((left_val_input, right_val_input), dim=1)
                    validation_inputs.append(concatted_input)
    ...
    def evaluate_model_roc(self, anchor_dataloader: DataLoader, validation_dataloader: DataLoader) -> Tuple[np.ndarray, np.ndarray]:
        log = logging.getLogger(__name__)
        ...
        with torch.no_grad():
            ...
            for left_anchor_inputs, right_anchor_inputs, batch_labels, _, _ in anchor_dataloader:
                anchor_inputs = []
            ...
            for left_val_inputs, right_val_inputs, batch_labels, _, _ in validation_dataloader:
                validation_inputs = []
    ...
    def evaluate_model_euclidean_distances_and_save(self, dataloader: DataLoader, model_filebasename: str, filename: str):
        ...
        with torch.no_grad():
            for left_inputs, right_inputs, _, left_img_filenames, _ in dataloader:
                inputs = []

                for left_input, right_input in zip(left_inputs, right_inputs):
                    concatted_input = torch.cat((left_input, right_input), dim=1)
                    inputs.append(concatted_input)
                ...
                row_labels.extend(left_img_filenames)
