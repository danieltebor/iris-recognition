...
def train_model(config: dict) -> str:
    ...
    weights, epoch_metadata = _fit_model(model, device, train_loader, val_loader, config)
    ...
    best_epoch = min(epoch_metadata, key=lambda x: x['val_loss'])['epoch']
...
def _fit_model(model: nn.Module, device: str, train_loader: DataLoader, val_loader: DataLoader, config: dict) -> tuple[dict, list[dict]]:
    scaler = GradScaler()
...
def _eval_angles(model: nn.Module, device: str, transform: transforms.Compose, model_dir: str, compare_with_frontal: bool, config: dict):
...
def _eval_tsne(model: nn.Module, device: str, transform: transforms.Compose, model_dir: str, config: dict):
    test_loader = _get_dataloader(
        data_dir=config['data']['test_dir'],
        transform=transform,
        config=config,
...
def _get_dataloader(data_dir: str,
                    transform: transforms.Compose,
                    config: dict,
                    is_triplet: bool,
                    shuffle: bool = False,
                    angles: list[str] = None) -> DataLoader:
    if is_triplet:
